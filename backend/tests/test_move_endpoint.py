...
@ pytest.mark.parametrize("old_status,new_status,expected_code,expected_msg", [
    ("SCHEDULED", "COMPLETED", "INVALID_TRANSITION", "Invalid transition SCHEDULED → COMPLETED"),
])
def test_invalid_transition(client, old_status, new_status, expected_code, expected_msg, monkeypatch):
...
def test_rate_limited(client):
    ...
    resp = client.patch("/api/admin/appointments/apt1/move", json={"status": "IN_PROGRESS", "position": 1})
    data = resp.get_json()
