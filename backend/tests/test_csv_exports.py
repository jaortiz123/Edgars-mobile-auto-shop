...
class TestCSVExports:
    ...
    def advisor_headers(self):
        ...
        return {
            'Authorization': 'Bearer mock_advisor_token',
            'Content-Type': 'application/json'
        }
    ...
    def technician_headers(self):
        ...
        return {
            'Authorization': 'Bearer mock_technician_token',
            'Content-Type': 'application/json'
        }
    ...
    def test_appointments_export_owner_access(self, mock_auth, client, owner_headers, sample_appointments_data):
    ...
    @patch('local_server.require_auth_role')
    def test_appointments_export_advisor_access(self, mock_auth, client, advisor_headers, sample_appointments_data):
    ...
    @patch('local_server.require_auth_role')
    def test_appointments_export_accountant_access(self, mock_auth, client, accountant_headers, sample_appointments_data):
    ...
    def test_appointments_export_rate_limit_enforced(self, mock_rate_limit, mock_auth, client, owner_headers):
    ...
    @patch('local_server.require_auth_role')
    @patch('local_server.rate_limit')
    def test_payments_export_rate_limit_enforced(self, mock_rate_limit, mock_auth, client, owner_headers):
    ...
    def test_appointments_export_invalid_date_format(self, mock_auth, client, owner_headers):
        ...
        response = client.get('/api/admin/reports/appointments.csv?from=invalid-date', headers=owner_headers)
    ...
    def test_appointments_export_invalid_status(self, mock_auth, client, owner_headers):
        ...
        response = client.get('/api/admin/reports/appointments.csv?status=INVALID_STATUS', headers=owner_headers)
        assert response.status_code == 400
        data = json.loads(response.data)
        assert data['error_code'] == 'INVALID_STATUS'

    @patch('local_server.require_auth_role')
    def test_appointments_export_valid_date_range(self, mock_auth, client, owner_headers, sample_appointments_data):
        ...
        with patch('local_server.db_conn') as mock_db_conn, \
            ...
            response = client.get('/api/admin/reports/appointments.csv?from=2024-01-01&to=2024-01-31', 
    ...
    @patch('local_server.require_auth_role')
    def test_appointments_csv_header_format(self, mock_auth, client, owner_headers, sample_appointments_data):
    ...
    @patch('local_server.require_auth_role')
    def test_payments_csv_header_format(self, mock_auth, client, owner_headers, sample_payments_data):
        ...
        with patch('local_server.db_conn') as mock_db_conn, \
            ...
            expected_header = [
                "ID", "Appointment ID", "Amount", "Payment Method", 
                "Transaction ID", "Payment Date", "Status"
            ]
            assert header == expected_header

    @patch('local_server.require_auth_role')
    def test_appointments_csv_data_format(self, mock_auth, client, owner_headers, sample_appointments_data):
    ...
    @patch('local_server.require_auth_role')
    def test_appointments_export_content_disposition(self, mock_auth, client, owner_headers, sample_appointments_data):
    ...
    @patch('local_server.require_auth_role')
    def test_payments_export_content_disposition(self, mock_auth, client, owner_headers, sample_payments_data):
    ...
    def test_appointments_export_audit_logging(self, mock_audit_log, mock_auth, client, owner_headers, sample_appointments_data):
    ...
    @patch('local_server.require_auth_role')
    @patch('local_server.audit_log')
    def test_payments_export_audit_logging(self, mock_audit_log, mock_auth, client, owner_headers, sample_payments_data):
    ...
    def test_csv_rfc4180_compliance(self, mock_auth, client, owner_headers):
        ...
        with patch('local_server.db_conn') as mock_db_conn, \
            ...
            first_row = next(csv_reader)
            assert first_row[6] == 'Doe, John "Johnny"'  # Should be properly unescaped by csv.reader
