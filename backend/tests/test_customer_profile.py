...
class _Cursor:
    ...
    def fetchone(self):
        ...
        if self.step == 1:
            ...
            return {"id": "cust-1", "name": "Customer One", "phone": "555-1111", "email": "one@example.com", "is_vip": False, "created_at": now - timedelta(days=400), "updated_at": now - timedelta(days=1)}
    ...
    def fetchall(self):
        ...
        if "FROM vehicles v" in self.last_sql:
            return [
                {"id": "veh-1", "license_plate": "PLT1", "year": 2020, "make": "Make", "model": "Model1", "total_spent": 500, "visits": 2},
                {"id": "veh-2", "license_plate": "PLT2", "year": 2021, "make": "Make", "model": "Model2", "total_spent": 400, "visits": 1},
            ]
        # step 4 appointments
        if "FROM appointments a" in self.last_sql:
            ...
            base_appts = [
                {
                    "id": "appt-1", "status": "COMPLETED", "start_ts": now - timedelta(days=30), "end_ts": now - timedelta(days=29, hours=22),
                    "total_amount": 500, "paid_amount": 500, "check_in_at": None, "check_out_at": None,
                    "vehicle_id": "veh-1", "license_plate": "PLT1", "year": 2020, "make": "Make", "model": "Model1",
                    "services": [{"id": "svc-1", "name": "Oil Change", "notes": None, "estimated_price": 100, "service_operation_id": None}] if self.want_details else None,
                    "payments": [{"id": "pay-1", "amount": 500, "method": "cash", "created_at": now - timedelta(days=29, hours=22)}] if self.want_details else None,
                    "messages": [{"id": "msg-1", "channel": "sms", "direction": "out", "body": "Done", "status": "SENT", "created_at": now - timedelta(days=29, hours=22)}] if self.want_details else None,
                },
                {
                    "id": "appt-2", "status": "SCHEDULED", "start_ts": now - timedelta(days=5), "end_ts": None,
                    "total_amount": 400, "paid_amount": 200, "check_in_at": None, "check_out_at": None,
                    "vehicle_id": "veh-2", "license_plate": "PLT2", "year": 2021, "make": "Make", "model": "Model2",
                    "services": [{"id": "svc-2", "name": "Brakes", "notes": None, "estimated_price": 400, "service_operation_id": None}] if self.want_details else None,
                    "payments": [{"id": "pay-2", "amount": 200, "method": "card", "created_at": now - timedelta(days=5)}] if self.want_details else None,
...
def test_profile_expanded_details(client, auth_headers, mock_db, monkeypatch):
    ...
    resp = client.get('/api/admin/customers/cust-1?include=appointmentDetails', headers=auth_headers())
    assert resp.status_code == 200
    body = resp.get_json()["data"]
    ...
    class _NFConn:
        ...
        def cursor(self):
            class _C:
                def __enter__(self): return self
                def __exit__(self, *a): pass
                def execute(self, sql, params=None): self.params=params; self.sql=sql
...
def test_profile_vip_overdue_logic(client, auth_headers, mock_db, monkeypatch):
    ...
    class _C2(_Cursor):
        def fetchone(self):
            if self.step == 1:
                now = datetime(2025, 8, 15, 12, 0, 0, tzinfo=timezone.utc)
                return {"id": "cust-2", "name": "Customer Two", "phone": None, "email": None, "is_vip": False, "created_at": now - timedelta(days=800), "updated_at": now - timedelta(days=1)}
            if "WITH appts AS" in self.last_sql:
                now = datetime(2025, 8, 15, 12, 0, 0, tzinfo=timezone.utc)
                return {
                    "total_spent": 6000,
    ...
    class _Conn2:
        def __enter__(self): return self
        def __exit__(self, *a): pass
        def cursor(self): return _C2(True)
    monkeypatch.setattr(srv, "safe_conn", lambda: (_Conn2(), False, None))
    resp = client.get('/api/admin/customers/cust-2?include=appointmentDetails', headers=auth_headers())
    assert resp.status_code == 200
