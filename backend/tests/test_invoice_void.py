...
def _make_invoice(conn, price_cents=10000, status='COMPLETED'):
    with conn.cursor() as cur:
        cur.execute("""INSERT INTO appointments (customer_id, vehicle_id, status, created_at)
                       VALUES (1,1,%s, now()) RETURNING id""", (status,))
        row = cur.fetchone()
        ...
        if price_cents:
            cur.execute("""INSERT INTO appointment_services (appointment_id, name, estimated_price, created_at)
                           VALUES (%s,'Svc',%s/100.0, now())""", (appt_id, price_cents))
    conn.commit()
...
def test_void_partially_paid_invoice(pg_container):
    ...
    client = srv.app.test_client()
    # pay part (4000)
    pay_resp = client.post(f"/api/admin/invoices/{invoice_id}/payments", json={'amountCents': 4000, 'method': 'cash'})
...
def test_void_paid_invoice_rejected(pg_container):
    ...
    client = srv.app.test_client()
    # pay full
    pay = client.post(f"/api/admin/invoices/{invoice_id}/payments", json={'amountCents': 5000, 'method': 'cash'})
