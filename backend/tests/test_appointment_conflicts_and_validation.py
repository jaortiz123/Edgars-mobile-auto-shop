...
def test_edit_conflict_same_tech(client, db_connection, future_start):
    ...
    payloadA = {"customer_id": "Cust A", "requested_time": _iso(future_start), "tech_id": tech_uuid, "license_plate": "ABC123", "end_ts": _iso(future_start + timedelta(hours=2))}
    ...
    payloadB = {"customer_id": "Cust B", "requested_time": _iso(future_start + timedelta(hours=3)), "tech_id": tech_uuid, "license_plate": "ABC123", "end_ts": _iso(future_start + timedelta(hours=5))}
    ...
    patch_payload = {"start": _iso(future_start)}  # move B to overlap A using default block duration
...
def test_create_validation_error_paid_exceeds_total(client, db_connection, future_start):
    ...
    body = r.get_json(); assert body and body['errors'][0]['code'] == 'VALIDATION_FAILED'


def test_edit_validation_error_paid_exceeds_total(client, db_connection, future_start):
    # Create valid appointment
    payload_ok = {"customer_id": "Cust D", "requested_time": _iso(future_start), "tech_id": '44444444-4444-4444-4444-444444444444', "total_amount": 100, "paid_amount": 20, "license_plate": "GHI789"}
    ...
    body = r_patch.get_json(); assert body and body['errors'][0]['code'] == 'VALIDATION_FAILED'
