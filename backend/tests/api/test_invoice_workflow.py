...
def test_invoice_full_workflow(client):
    ...
    start_ts = (datetime.now(timezone.utc) + timedelta(minutes=1)).isoformat().replace('+00:00', 'Z')
    create_resp = client.post('/api/admin/appointments', json={
    ...
    svc_resp = client.post(f'/api/appointments/{appt_id}/services', json={
        'name': 'WF Service',
        'estimated_price': 50,  # dollars -> 5000 cents
    })
    assert svc_resp.status_code in (HTTPStatus.OK, HTTPStatus.CREATED), svc_resp.get_data(as_text=True)
    ...
    if amount_due_cents > 0:
        pay_resp = client.post(f'/api/admin/invoices/{invoice_id}/payments', json={
            'amountCents': amount_due_cents,
            'method': 'cash'
        })
