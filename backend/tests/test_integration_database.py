...
class TestContainerizedDatabase:
    ...
    def test_seed_data_loaded(self, db_connection):
        ...
        with db_connection.cursor() as cur:
            ...
            assert appointment_count == 10, f"Expected 10 appointments, got {appointment_count}"
            
            # Check appointment services
            cur.execute("SELECT COUNT(*) as count FROM appointment_services")
            service_count = cur.fetchone()['count']
            assert service_count == 16, f"Expected 16 services, got {service_count}"
    ...
    def test_complex_joins_work(self, db_connection):
        ...
        with db_connection.cursor() as cur:
            ...
            for row in results:
                ...
                assert row['customer_email'] is not None
    ...
    def test_appointment_service_relationships(self, db_connection):
        ...
        with db_connection.cursor() as cur:
            ...
            assert appointment_with_services is not None, "Should have at least one appointment with services"
            ...
            cur.execute(
                "SELECT * FROM appointment_services WHERE appointment_id = %s",
                (appointment_id,)
            )
...
class TestDatabaseRobustness:
    ...
    def test_resource_cleanup_verification(self, pg_container):
        ...
        assert current_postgres_processes <= initial_postgres_processes + 10, \
...
class TestAdvancedRobustness:
    ...
    def test_data_consistency_verification(self, db_connection):
        ...
        with db_connection.cursor() as cur:
            ...
            result = cur.fetchone()
            assert result["orphaned_vehicles"] == 0, "Found vehicles with invalid customer references"
            assert result["orphaned_appointments_customers"] == 0, "Found appointments with invalid customer references"
            assert result["orphaned_appointments_vehicles"] == 0, "Found appointments with invalid vehicle references"
            assert result["orphaned_services"] == 0, "Found services with invalid appointment references"
