...
def test_partial_payment_sets_partially_paid(pg_container):
    ...
    # pay half (5000)
    resp = client.post(
        f"/api/admin/invoices/{invoice_id}/payments", json={"amountCents": 5000, "method": "card"}
    )
    data = resp.get_json()
...
def test_full_payment_sets_paid(pg_container):
    ...
    client = srv.app.test_client()
    resp = client.post(
        f"/api/admin/invoices/{invoice_id}/payments", json={"amountCents": 4200, "method": "cash"}
    )
    data = resp.get_json()
    assert resp.status_code == 201, data
...
def test_overpayment_rejected(pg_container):
    ...
    client = srv.app.test_client()
    resp = client.post(
        f"/api/admin/invoices/{invoice_id}/payments", json={"amountCents": 4000, "method": "card"}
    )
    data = resp.get_json()
...
def test_payment_on_paid_invoice_rejected(pg_container):
    ...
    # pay full first
    resp1 = client.post(
        f"/api/admin/invoices/{invoice_id}/payments", json={"amountCents": 1000, "method": "cash"}
    )
    assert resp1.status_code == 201, resp1.get_json()
    # attempt second payment
    resp2 = client.post(
        f"/api/admin/invoices/{invoice_id}/payments", json={"amountCents": 100, "method": "cash"}
    )
