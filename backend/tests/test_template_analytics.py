...
def test_empty_range_returns_zeroes(client, db_connection, now_utc):
    r = client.get('/api/admin/analytics/templates?range=7d')
    assert r.status_code == HTTPStatus.OK
    body = r.get_json(); assert body
...
def seed_event(cur, template_slug, sent_at, channel=None, user_id=None):
    ...
    tpl = cur.fetchone(); assert tpl, template_slug
...
def test_basic_aggregation(client, db_connection):
    ...
    templates = [
        'vehicle_ready_sms','vehicle_ready_sms','vehicle_ready_sms','thanks_followup_email','thanks_followup_email'
    ]
