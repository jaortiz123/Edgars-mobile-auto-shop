...
def test_get_single_appointment_details(client, db_connection):
    ...
    op_id = "svc-op-align"
    cur.execute(
        "INSERT INTO service_operations (id, name, default_price, category) VALUES (%s,'Alignment',59.99,'Chassis')",
        (op_id,),
    )
    cur.execute(
        "INSERT INTO appointments (customer_id, vehicle_id, status, start_ts) VALUES (%s,%s,'SCHEDULED', now()) RETURNING id",
        (cust_id, veh_id),
    )
