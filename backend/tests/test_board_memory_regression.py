...
def test_board_shows_memory_created_appointment(client):
    ...
    r = client.post("/api/admin/appointments", json={"customer_name": "Mem Cust", "license_plate": "XYZ123"})
    ...
    assert any(c["id"] == appt_id for c in board.get("cards", [])), "Memory-created appointment missing from board"
