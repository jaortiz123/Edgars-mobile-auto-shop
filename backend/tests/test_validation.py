...
def test_validate_paid_exceeds_total():
    now = datetime.now(timezone.utc) + timedelta(minutes=5)
    res = validate_appointment_payload(
        {"start_ts": now.isoformat(), "total_amount": 10, "paid_amount": 20}
    )
...
def test_find_conflicts_returns_ids():
    ...
    conflicts = find_conflicts(
        conn, tech_id="t1", vehicle_id=9, start_ts=now, end_ts=now + timedelta(hours=1)
    )
...
def test_find_conflicts_vehicle():
    ...
    conflicts = find_conflicts(
        conn, tech_id=None, vehicle_id=5, start_ts=now, end_ts=now + timedelta(hours=1)
    )
