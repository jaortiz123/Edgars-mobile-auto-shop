...
VALUES
  ('oil-change-synthetic','oil-change-synthetic','Full Synthetic Oil Change','Maintenance','{oil,filter,synthetic,premium}',0.50,60.00,'{quick-service,no-appointment}',TRUE,NULL,'EXPRESS',1),
  ('tire-rotation-awd','tire-rotation-awd','AWD Tire Rotation','Maintenance','{tire,rotation,all-wheel-drive}',0.75,90.00,'{appointment-recommended}',TRUE,NULL,'STD',2),
  ('brake-inspection-old','brake-inspection-old','Basic Brake Inspection','Brakes','{brake,inspection}',0.50,60.00,'{}',FALSE,'brake-inspection-comprehensive','STD',2),
  ('brake-inspection-comprehensive','brake-inspection-comprehensive','Comprehensive Brake Inspection','Brakes','{brake,inspection,safety,comprehensive}',0.75,90.00,'{free-with-service}',TRUE,NULL,'STD',2),
  ('brake-pad-rotor-front','brake-pad-rotor-front','Front Brake Pads & Rotors','Brakes','{pads,rotors,brake-noise}',2.00,280.00,'{safety-critical}',TRUE,NULL,'BRAKE-STD',3),
  ('engine-diagnostic-advanced','engine-diagnostic-advanced','Advanced Engine Diagnostics','Diagnostics','{engine,diagnostic,troubleshoot,check-engine}',2.00,300.00,'{requires-test-drive}',TRUE,NULL,'DIAG-A',4),
  ('electrical-diagnostic','electrical-diagnostic','Electrical System Diagnostic','Diagnostics','{battery-drain,short,no-start}',1.50,225.00,'{intermittent-possible}',TRUE,NULL,'DIAG-B',4),
  ('transmission-service-cvt','transmission-service-cvt','CVT Transmission Service','Transmission','{transmission,cvt,fluid,filter}',2.00,280.00,'{dealer-recommended}',TRUE,NULL,'TRANS-ADV',4),
  ('ac-recharge-r134a','ac-recharge-r134a','A/C Recharge R-134a','HVAC','{air-conditioning,ac,recharge,r134a}',1.00,130.00,'{seasonal-service}',TRUE,NULL,'HVAC-STD',3),
  ('four-wheel-alignment','four-wheel-alignment','4-Wheel Alignment','Suspension','{alignment,pull,tire-wear}',1.00,130.00,'{post-suspension}',TRUE,NULL,'SUSP-ALIGN',3),
  ('strut-assembly-pair','strut-assembly-pair','Front Strut Assembly (Pair)','Suspension','{ride-height,clunk,strut}',3.50,507.50,'{alignment-recommended}',TRUE,NULL,'SUSP-ADV',4),
  ('cv-axle-front','cv-axle-front','Front CV Axle Replacement (Each)','Drivetrain','{cv,clicking,boot}',2.00,280.00,'{alignment-recommended}',TRUE,NULL,'DRIVE-STD',3),
  ('cabin-filter-replacement','cabin-filter-replacement','Cabin Air Filter Replacement','Maintenance','{filter,cabin,hvac}',0.30,33.00,'{upsell-hvac}',TRUE,NULL,'EXPRESS',1),
  ('spark-plug-replacement','spark-plug-replacement','Spark Plug Replacement (4-cyl)','Engine','{tune-up,spark,misfire}',1.50,202.50,'{engine-hot}',TRUE,NULL,'ENG-TUNE',3),
  ('water-pump-replacement','water-pump-replacement','Water Pump Replacement','Cooling','{overheat,coolant-leak,bearing-noise}',2.50,362.50,'{timing-belt-related}',TRUE,NULL,'COOL-ADV',3),
  ('fuel-injector-clean','fuel-injector-clean','Fuel Injector Cleaning (Machine-Assisted)','Fuel','{injector,rough-idle,mpg}',1.00,130.00,'{emissions-related}',TRUE,NULL,'FUEL-SVC',2),
  ('cat-converter-replacement','cat-converter-replacement','Catalytic Converter Replacement','Emissions','{catalytic,P0420,emissions}',2.50,400.00,'{theft-prone,regulatory}',TRUE,NULL,'EMISS-ADV',4),
  ('hybrid-battery-service','hybrid-battery-service','Hybrid Battery Service','Electrical','{hybrid,battery,high-voltage}',3.00,540.00,'{certification-required}',TRUE,NULL,'EV-HIGH',5),
  ('ev-inverter-coolant-flush','ev-inverter-coolant-flush','EV Inverter/Charger Coolant Service','Electrical','{inverter,coolant,ev}',1.25,212.50,'{oem-spec-only}',TRUE,NULL,'EV-COOL',4),
  ('adas-front-camera-cal','adas-front-camera-cal','ADAS Front Camera Calibration','Safety','{ADAS,camera,calibration}',2.00,350.00,'{level-floor-required,post-windshield}',TRUE,NULL,'ADAS-CAL',5),
  ('diesel-dpf-regeneration','diesel-dpf-regeneration','Diesel DPF Forced Regeneration','Diesel','{DPF,regen,soot}',1.00,165.00,'{hot-exhaust}',TRUE,NULL,'DIESEL-DIAG',4),
  ('safety-inspection','safety-inspection','General Safety Inspection','Safety','{inspection,safety,pre-trip}',0.75,90.00,'{paperwork}',TRUE,NULL,'SAFETY',2),
  ('smog-check-obd2','smog-check-obd2','Smog Check (OBD-II)','Emissions','{smog,emissions,obd2}',0.75,97.50,'{regulatory,state-specific}',TRUE,NULL,'EMISS-TEST',3)
