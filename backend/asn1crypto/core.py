    ...
    def dump(self, force=False):
        ...
        if self._header is None or force:
            ...
            if self.explicit is not None:
                for class_, tag in self.explicit:
                    self._header = (
                        _dump_header(class_, 1, tag, self._header + self._contents) + self._header
                    )
                ...
                for index, (_, _, params) in enumerate(self._fields):
                    if "default" in params:
                        if cls._precomputed_specs[index]:
                            field_name, field_spec, value_spec, field_params, _ = (
...
class AbstractTime(AbstractString):
    ...
    def _parsed_time(self):
        ...
        if groups["zulu"]:
            ...
            tz = create_timezone(
                sign * timedelta(hours=int(groups["dhour"]), minutes=int(groups["dminute"] or 0))
            )
...
def _basic_debug(prefix, self):
    ...
    if self._trailer:
        print(
            "%s  Trailer: 0x%s" % (prefix, binascii.hexlify(self._trailer or b"").decode("utf-8"))
        )
...
def _build(
    class_, method, tag, header, contents, trailer, spec=None, spec_params=None, nested_spec=None
):
    """
