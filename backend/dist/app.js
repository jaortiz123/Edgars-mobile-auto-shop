...
async function seedIfEmpty() {
    ...
    if (Number(rows[0].count) === 0) {
        await db.query(`INSERT INTO services (name, description, duration_minutes, base_price)
       VALUES
...
// });
app.get('/', (req, res) => res.json({ status: 'Backend is live', timestamp: new Date().toISOString() }));
...
app.get('/debug/seed-status', async (req, res) => {
    const [{ rows: services }, { rows: customers }, { rows: appointments }] = await Promise.all([
