...
def upgrade():
    # Create enums
    invoice_status = postgresql.ENUM('DRAFT', 'SENT', 'PARTIALLY_PAID', 'PAID', 'VOID', name=INVOICE_STATUS_ENUM)
    payment_method = postgresql.ENUM('CASH', 'CARD', 'CHECK', 'ACH', 'OTHER', name=PAYMENT_METHOD_ENUM)
    ...
    op.create_foreign_key('fk_invoices_appointment', 'invoices', 'appointments', ['appointment_id'], ['id'], ondelete='SET NULL')
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['service_operation_id'], ['service_operations.id'], name='fk_invoice_line_items_service_op', ondelete='SET NULL'),
    ...
    op.create_index('idx_invoice_line_items_invoice', 'invoice_line_items', ['invoice_id', 'position'])
    ...
    op.create_table(
        ...
        sa.Column('notes', sa.Text(), nullable=True),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['invoice_id'], ['invoices.id'], name='fk_payments_invoice', ondelete='CASCADE'),
