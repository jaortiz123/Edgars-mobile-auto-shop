...
# PHASE A STEP 4: Monolith Shimming
# Optional factory component integration for gradual migration
USE_FACTORY_COMPONENTS = os.getenv("USE_FACTORY_COMPONENTS", "false").lower() in ("true", "1")
if USE_FACTORY_COMPONENTS:
    print("[SHIMMING] Using factory-extracted middleware components")
    try:
        from backend.middleware.cors import register_cors_middleware
        from backend.middleware.request_meta import register_request_meta_middleware
        from backend.middleware.security_headers import register_security_headers_middleware
        from backend.middleware.tenant import register_tenant_middleware

        # Register factory middleware (this will add to the existing app)
        register_request_meta_middleware(app)
        register_tenant_middleware(app)
        register_cors_middleware(app)
        register_security_headers_middleware(app)

        print("[SHIMMING] Factory middleware registered successfully")

        # Set flag to skip embedded middleware registration later
        app.config["_FACTORY_MIDDLEWARE_ACTIVE"] = True

    except Exception as e:
        print(f"[SHIMMING] Failed to load factory components: {e}")
        print("[SHIMMING] Falling back to embedded monolith middleware")
        USE_FACTORY_COMPONENTS = False
        app.config["_FACTORY_MIDDLEWARE_ACTIVE"] = False
else:
    print("[SHIMMING] Using embedded monolith middleware (default)")
    app.config["_FACTORY_MIDDLEWARE_ACTIVE"] = False
...
ALLOWED_ORIGINS = [
    ...
    for o in os.environ.get(
        ...
        "http://localhost:5173,http://mobile-auto-shop-staging-b928aa27.s3-website-us-west-2.amazonaws.com",
    ).split(",")
    if o.strip()
]
ALLOWED_METHODS = "GET,POST,PATCH,PUT,DELETE,OPTIONS"
ALLOWED_HEADERS = "Content-Type,Authorization,If-Match,If-None-Match,Origin,Accept,X-Requested-With"
...
def _add_cors_headers(resp):
    ...
    SHIMMING: Skip if factory middleware is active.
    """
    # Skip if factory components are handling this
    if app.config.get("_FACTORY_MIDDLEWARE_ACTIVE", False):
        return resp
...
try:
    ...
    if not getattr(app, "_API_CONSISTENCY_INSTALLED", False):  # type: ignore[attr-defined]
        ...
        def _standardize_json_envelope(resp: Response):
            try:
                # Skip envelope for board endpoint (returns raw shape)
                if request.path == "/api/admin/appointments/board":
                    resp.headers.setdefault("X-Correlation-Id", getattr(g, "correlation_id", "?"))
                    return resp
                resp.headers.setdefault("X-Correlation-Id", getattr(g, "correlation_id", "?"))
...
def _mark_first_byte(resp):  # pragma: no cover - simple timestamp setter
    """Set FIRST_BYTE_TS the first time after_request runs.
    ...
    if not conn and use_memory:
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                print(f"[DEBUG] frm={frm}, to={to}, condition={bool(frm or to)}")
                if frm or to:
                    where = ["1=1"]
                    if frm:
              ...
              SELECT a.id::text,
                  ...
                  COALESCE(a.total_amount, 0) AS price
                    ...
                    # No carryover logic for explicit date ranges
...
def move_card(appt_id: str):
    print(f"[MOVE_DEBUG] move_card called with appt_id='{appt_id}'")
    # Step 1: Enforce Advisor-level authentication
...
def patch_appointment(appt_id: str):
    print(f"[PATCH_DEBUG] patch_appointment called with appt_id='{appt_id}'")
    try:
        # Step 1: Enforce authentication with role requirement
        auth_payload = require_auth_role("Advisor")
...
def get_customer_notes(cust_id: str):
    ...
    if not conn and use_memory:
        ...
        return _ok({"notes": [], "total": 0, "limit": limit_val, "offset": offset_val})
            ...
            notes.append(
                {
                    ...
                    "isAlert": (
                        bool(row.get("is_alert")) if row.get("is_alert") is not None else False
                    ),
                    "createdAt": (
                        row["created_at"].isoformat() + "Z" if row.get("created_at") else ""
                    ),
                }
            )
        else:  # Tuple cursor
    ...
    return _ok({"notes": notes, "total": total_count, "limit": limit_val, "offset": offset_val})
...
def _visits_rows_to_payload(rows):
    ...
    for r in rows:
        ...
        for service in services:
            ...
            service_category = (
                service.get("category", "").lower() if service.get("category") else ""
            )

            # Determine service type
            service_type = "Other"
            if (
                "diagnostic" in service_name
                or "diagnosis" in service_name
                or service_category == "diagnostic"
            ):
                ...
                for keyword in ["replacement", "part", "solenoid", "filter", "fluid"]
            ) or service_category in ["transmission", "engine", "brake"]:
                service_type = "Parts"
            ...
            if notes:
                ...
                warranty_match = re.search(
                    r"warranty:?\s*(\d+)\s*(year|month)s?[/,\s]*(\d+[,\d]*)\s*miles?",
                    notes,
                    re.IGNORECASE,
                )
                if warranty_match:
                    years = (
                        int(warranty_match.group(1))
                        if warranty_match.group(2).lower() == "year"
                        else int(warranty_match.group(1)) // 12
                    )
                    ...
                    if r.get("end_ts"):
                        try:
                            ...
                            warranty_expiry_date = service_date.replace(
                                year=service_date.year + years
                            )
        ...
        visit = {
            ...
            "checkInAt": r.get("check_in_at").isoformat() if r.get("check_in_at") else None,
            "checkOutAt": r.get("check_out_at").isoformat() if r.get("check_out_at") else None,
            "vehicle": " ".join(str(x) for x in [r.get("year"), r.get("make"), r.get("model")] if x)
            or "Vehicle",
            "plate": r.get("license_plate"),
            ...
            "has_warranty_active": any(
                s["warranty"]["status"] == "Active" for s in enhanced_services
            ),
            "major_services": [
                s["name"]
                for s in enhanced_services
                if s["service_type"] in ["Parts", "Service"] and s.get("estimated_price", 0) > 200
            ],
        }
...
# ----------------------------------------------------------------------------
# Global CORS preflight handler (Gate C)
# ----------------------------------------------------------------------------
...
def _cors_preflight(path):
    ...
    # If factory middleware is active, let it handle CORS preflight
    if app.config.get("_FACTORY_MIDDLEWARE_ACTIVE", False):
        # Return early - factory middleware will add headers
...
# SPRINT 6 HOTFIX: Simple test endpoint to verify routing
@app.route("/api/v1/test", methods=["GET"])
def test_v1():
    return _ok({"message": "Test endpoint works"})
