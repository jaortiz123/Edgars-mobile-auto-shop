...
def lambda_handler(event, context):
    ...
    # POST /appointments - Create appointment
        ...
        try:
            ...
            return {
                ...
                "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"},
                "body": json.dumps(
                    {"id": appointment_id, "message": "Appointment created successfully"}
                ),
            }
        finally:
            conn.close()
    ...
    # No route matched
        ...
        return {
            "statusCode": 404,
            "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"},
            "body": json.dumps({"error": "Route not found", "route_received": route_key}),
        }
...
def send_confirmation_notification(appointment_id, body, customer_id):
    ...
    try:
        ...
        logger.info(
            f"Confirmation notification sent for appointment {appointment_id}: {response['MessageId']}"
        )
