 ...
 * @property {number} totalRevenue - Total revenue for today
 * @property {number} paidAmount - Amount already paid
 * @property {number} unpaidAmount - Amount pending payment
 ...
 */
export function subscribeToRevenueUpdates(callback) {
 ...
 */
function notifySubscribers(revenueData) {
 ...
 * @returns {string} Formatted currency string
 */
export function formatDetailedCurrency(amount, includeDecimals = false) {
  return new Intl.NumberFormat('en-US', {
    style: 'currency',
    currency: 'USD',
