...
export default function AppointmentCardRobust({ card, onOpen, onMove, onQuickReschedule }: AppointmentCardProps) {
  ...
  // Memoized urgency level calculation - only if validatedCard exists
  ...
  const ariaLabel = useMemo(() => {
    ...
    return withCardErrorBoundary(
      () => CardAccessibility.generateAccessibleLabel(validatedCard, cardState),
      'Appointment card',
      'Error creating ARIA label'
    );
  }, [validatedCard, cardState]);

  // Check for urgent notifications - only if validatedCard exists
  ...
  // Drag and drop configuration - only if validatedCard exists
  const dragConfig = useMemo(() => {
    if (!validatedCard) return { type: 'appointment', item: { id: '', type: 'SCHEDULED' } };
    return {
      type: 'appointment',
  ...
  // Memoized handlers - only if validatedCard exists
      ...
      withCardErrorBoundary(() => {
        setCardState(prevState => {
          ...
          if (isStartingSoon(newMinutesUntil) && !prevState.notifiedLate) {
            withCardErrorBoundary(
              () => notifyUpcoming(validatedCard.id),
              undefined,
  ...
  }, [validatedCard, urgencyLevel, isOverdue]);
  ...
  // Memoized countdown text - only if validatedCard exists
