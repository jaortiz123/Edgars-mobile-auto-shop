...
export const ServiceCatalogModal: React.FC<ServiceCatalogModalProps> = ({ open, initialSelected, onConfirm, onClose }) => {
  ...
  const performSearch = useCallback(async (term: string) => {
    ...
    try {
      ...
      const resp = await fetch(`${API_BASE}/admin/service-operations?${params.toString()}`, { signal: controller.signal });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-6" role="dialog" aria-modal="true" aria-labelledby="svc-cat-title">
      <div className="w-full max-w-2xl rounded bg-white shadow-xl flex flex-col max-h-[80vh]">
        <div className="border-b px-4 py-3 flex items-center gap-3">
          <h2 id="svc-cat-title" className="text-lg font-semibold flex-1">Add Services</h2>
          <button onClick={onClose} className="text-sm px-2 py-1 border rounded" type="button">Close</button>
        </div>
        <div className="p-4 pt-3 space-y-3">
          ...
          <div className="text-xs text-gray-500 flex items-center gap-2">
            <span>{loading ? 'Loading…' : error ? 'Error loading services' : results.length ? `${results.length} result${results.length!==1?'s':''}` : 'Enter at least 2 characters to search'}</span>
            {draftSelected.length > 0 && <span className="ml-auto font-medium">{draftSelected.length} selected</span>}
          </div>
          <div className="border rounded overflow-hidden min-h-[180px]">
            <ul ref={listRef} className="max-h-64 overflow-auto divide-y" aria-label="Service results">
              {error && (
                <li className="p-3 text-red-600 flex justify-between items-center">
                  <span>{error}</span>
