...
describe('NotificationService Coverage Tests', () => {
  ...
  const mockLocalStorage = {
    ...
    // Set a very long retention period to avoid expiration issues during tests
    notificationService.updateConfig({
      retentionPeriod: 24 * 60 * 60 * 1000 * 365, // 1 year
      rateLimitPerType: 100 // Increase rate limit for tests
    });
  });
  ...
  describe('Core Notification Functions', () => {
    ...
    it('should handle rate limiting', () => {
      ...
      // Temporarily set a very low rate limit for this test
      notificationService.updateConfig({ rateLimitPerType: 3 });
      ...
      const successfulIds = ids.filter(id => id !== '');
      const droppedIds = ids.filter(id => id === '');

      // Should have some successful and some dropped notifications
      expect(successfulIds.length).toBeGreaterThan(0);
      expect(successfulIds.length).toBeLessThanOrEqual(3);
      expect(droppedIds.length).toBeGreaterThan(0);
      expect(successfulIds.length + droppedIds.length).toBe(10);

      // Reset rate limit back to test default
      notificationService.updateConfig({ rateLimitPerType: 100 });
    });
